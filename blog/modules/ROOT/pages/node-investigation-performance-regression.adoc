= Noble Node: Investigating a performance regression
Guillaume Grossetie <ggrossetie@yuzutech.fr>
:page-revdate: 2020-10-21 12:00
...
He quickly identified that `Asciidoctor.convert` was significantly slower.
...
In this case, it would be interesting to compare the two flamegraphs, one when running on Node 10 and another one running on Node 12.
We should be able to identify where the time is spent and which calls are slower.
...
I give you a clue, take a close look at the dark orange blocks.
...
> Array.p.splice() returns a new array. array-splice.tq has a fast path for that (ExtractFastJSArray, defined in src/codegen/code-stub-assembler.cc)
